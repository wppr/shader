...
vec3				sun_light_radiance=vec3(2.5);
uniform vec3 				eyepos;
...
//   vec4 Normalraw = texture(gbuffer_normal, texcoord);
//   vec3 N=vec3(decode(Normalraw.xy),decode(Normalraw.zw),0.0);

//   vec4 Normalraw2 = texture(gbuffer_normal2, texcoord);
//   N.z=decode(Normalraw2.xy);
//   N=N*2.0-1.0;
//   return N;
// }
...
void main() {
	...
	float spe=texture(gbuffer_specular,texcoord).x;
	roughness=(1.0-spe)*0.3+0.4;
	...
	if(0.0025+depthmap<shadowcoord.z/shadowcoord.w)
		shadow_term=0.3;
	...
	result += integrate_directional_light(NdotL, NdotV, NdotH, VdotH, f0, albedo, alpha)*shadow_term*ao_term ;
	//result += albedo * integrate_ambient_diffuse(mat3(T, B, N)) * ao_term;
	result+=0.2*albedo * prefilter_diffuse(N)*ao_term;
	result += integrate_ambient_specular(mat3(X, Y, R), alpha) * dfg_term * ao_term;

    hdr_image_out = result*1.5;
    vec3 q=hdr_image_out;
    float gamma=1.0/2.2;
